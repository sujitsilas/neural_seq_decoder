...
The dataset includes 8,800 training sentences, 880 validation sentences, and 1,200 test sentences, recorded across 24 days over about 4 months. We report performance on the validation set as test labels are held out for benchmark competition.
...
We train the model with the Adam optimizer ($\beta_1=0.9, \beta_2=0.999$, $\epsilon=0.1$, larger than PyTorch default). The learning rate decays from 0.02 to 0.002 over 10,000 batches. We use a batch size of 64 sentences with data augmentation including white noise ($\sigma=0.8$) and constant offset ($\sigma=0.2$). Regularization includes L2 weight decay (1e-5) and dropout (0.4). Training takes about 30 minutes for 10,000 batches on an NVIDIA RTX 3090.

The baseline model contains about 41.0M parameters. The GRU layers account for 39.4M parameters, with the day-specific transformation layers adding 1.6M and the output layer contributing 42K.
...
Model 4 uses the same 5-layer unidirectional GRU architecture as the baseline (1024 hidden units, orthogonal weight initialization) but with an expanded input dimension of 768. The increased input dimension affects the first GRU layer and the day-specific transformation layers (24 days × 768×768 affine transformations). We train with SGD using Nesterov accelerated gradient ($momentum=0.9$), a step learning rate schedule ($0.1 \rightarrow 0.01$ at batch 5000), and coordinated dropout at 15\% (higher than Model 2's 10\% due to the larger feature space). Other settings match Model 2: batch size 64, gradient clipping max norm 5.0, and standard augmentations (white noise $\sigma=0.8$, constant offset $\sigma=0.2$, Gaussian smoothing $\sigma=2.0$). The model has about 75.6M parameters, an 84\% increase over the baseline's 41.0M due to the tripled input dimension.
...
\toprule
Model & Description & Best PER & Final PER & Improvement & Parameters \\
\midrule
Model 1 & Baseline GRU + Adam & 21.84\% & 21.84\% & — & 41.0M \\
Model 2 & SGD + Momentum + Dropout & \textbf{19.66\%} & 19.71\% & \textbf{10.0\%} & 41.0M \\
Model 3 & Log + Layer Normalization & 21.54\% & 21.60\% & 1.4\% & 41.0M \\
Model 4 & Delta Features + Nesterov & 19.75\% & 19.78\% & 9.6\% & 75.6M \\
\bottomrule
\end{tabular}
\end{table}
...
Our models have key advantages: fewer parameters than transformers, faster inference due to unidirectional processing, and simpler training without test-time adaptation.
...
Our central finding is that careful optimization—SGD with momentum, step learning rate scheduling, and coordinated dropout—yields performance improvements (10\%) comparable to architectural innovations in recent work. This challenges the field's emphasis on architectural novelty over systematic hyperparameter optimization. The results show that training methodology matters as much as architectural choices for neural speech decoding.
...
Finally, coordinated dropout—where the same features are masked across all time steps—is well-suited to multi-electrode recordings. This forces models to learn representations robust to missing channels, a realistic scenario in chronic BCI deployments where electrode quality may degrade over time.
...
This work was completed as the final project for ECE C243A: Brain-Computer Interfaces (Fall 2025) at UCLA, taught by Prof. Jonathan C. Kao. I thank Prof. Kao and the teaching assistants (Ebrahim Feghhi, Shashwat Athreya, Yunus Turali) for their guidance throughout the course. I acknowledge the organizers of the Brain-to-Text Benchmark '24 for releasing the dataset and baseline code. Computational resources were provided by the UCLA Hoffman2 Cluster. Most importantly, I express deep gratitude to the clinical trial participant whose generous contribution of time and neural data made this research possible.
